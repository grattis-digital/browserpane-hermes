...
const tools = [
  ...
  { name: 'pane_flow', description: 'Run up to 4 verified role/name stages (8 inputs) on the existing tab in one replay-safe call. Each stage captures current state; non-final stages require wait. Unique targets only. Stops on changed/missing/ambiguous targets, popup, dialog, partial input or failed wait. Ordinary Playwright actionability; no CSS, JS, forced input, stealth or new tabs.',
    inputSchema: object({ tab: common.tab, lease: string(80), request: integer(1, 2147483647),
